...
exports.listBooks = async ({ page, pageSize, name, order, isAdmin }) => {
  ...
  return repo.listBooks({ active: isAdmin ? undefined : true, name, order, limit, offset });
};
...
exports.listBookDetails = ({ bookId, isAdmin }) => repo.listBookDetails({ bookId, onlyActive: !isAdmin });

exports.readBookDetailWithAccess = async ({ userId, bookDetailId, isAdmin }) => {
  // Check unlock policy
  const rows = await repo.getUnlockRows({ userId, bookDetailId, isAdmin });
  if (!rows.length && !isAdmin) throw Object.assign(new Error('Access denied'), { status: 403 });
  ...
  if (blocked && !isAdmin) throw Object.assign(new Error('Book not unlocked yet'), { status: 403 });
