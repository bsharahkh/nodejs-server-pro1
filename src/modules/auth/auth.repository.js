...
exports.setVerifyCode = async (email, code) => {
  await pool.query('UPDATE users SET verify_code=$1 WHERE email=$2', [code, email]);
};

exports.verifyEmail = async (email) => {
  await pool.query('UPDATE users SET verified=TRUE, verify_code=NULL WHERE email=$1', [email]);
};
...
exports.updatePasswordByEmail = async (email, hash) => {
  await pool.query('UPDATE users SET password=$1, reset_code=NULL WHERE email=$2', [hash, email]);
};
...
exports.ensureUserWithGoogle = async (email) => {
  ...
  try {
    ...
    let userId;
    if (!u.rows.length) {
      const ins = await client.query('INSERT INTO users (email, verified) VALUES ($1, TRUE) RETURNING id', [email]);
